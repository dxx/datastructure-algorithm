...
class MinTree {
  ...
  constructor(vertexes: string[], edges: number[][]) {
    ...
    let initialVertexes = new Array<string>(numOfVertex);
    let initialMatrix = new Array<Array<number>>(numOfVertex);
    for (let i = 0; i < numOfVertex; i++) {
      initialVertexes[i] = vertexes[i];
      initialMatrix[i] = new Array<number>(numOfVertex);
  ...
  kruskal(): void {
    ...
    let endPosits = new Array<number>(this.graph.numOfEdge);
   ...
   * @param vertex 顶点
   */
  getVertexPosit(vertex: string): number {
   ...
   */
  getEndPosit(posits: number[], i: number): number {
    while (posits[i]) {
      i = posits[i];
    }
    return i;
...
class Graph {
  ...
  constructor(vertexes: string[], matrix: number[][], numOfEdge: number) {
