...
function decompose(nums: number[], start: number, end: number): void {
  if (start < end) {
...
function merge(nums: number[], left: number, mid: number, right: number): void {
  let temp = new Array<number>(right - left + 1); // 临时保存元素的数组
  let i = left; // 左边有序序列的起始下标
  let j = mid + 1; // 右边有序列表的起始下标
  let t = 0; // temp 当前下标
