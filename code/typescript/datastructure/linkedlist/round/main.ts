 ...
 * 插入结点
 */
function insertNode(headNode: PersonNode, newNode: PersonNode) {
  // 判断是否第一次插入
 ...
 * 删除指定结点，返回头结点
 */
function deleteNode(headNode: PersonNode, node: PersonNode) {
  // 没有结点 或者 只有一个头结点
  ...
  let tempNode: PersonNode | null = headNode.next;
  let isExist = false;
  while (true) {
 ...
 * 打印循环链表的信息
 */
function printRoundNodeInfo(headNode: PersonNode) {
  if (!headNode.next) {
    console.log("该链表没有节点");
    return;
  }
