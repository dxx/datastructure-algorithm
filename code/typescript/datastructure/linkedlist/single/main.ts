 ...
 */
function insertAtTail(headNode: HeroNode, newNode: HeroNode) {
 ...
 */
function sortInsertByNo(headNode: HeroNode, newNode: HeroNode) {
 ...
 * 删除指定结点
 */
function deleteNode(headNode: HeroNode, node: HeroNode) {
  let tempNode = headNode;
 ...
 * 打印单链表结点内容
 */
function printHeadNodeInfo(headNode: HeroNode) {
  if (!headNode.next) {
    console.log("该链表没有节点");
    return;
  }
  let str = "[";
  let tempNode: HeroNode | null = headNode.next;
  while (tempNode !== null) {
