...
class BinarySortTree {
  root: BinaryTreeNode | null; // 树的根节点
  ...
  _add(root: BinaryTreeNode, node: BinaryTreeNode) {
    // 要添加的节点小于根节点
    if (node.no < root.no) {
  ...
  search(no: number): [BinaryTreeNode | null, BinaryTreeNode] | null {
    if (this.root === null) {
      return null;
    }
    // 查找的节点就是根节点
   ...
   * 返回查找到的父节点和查找到的节点
   */
  _recursionSearch(node: BinaryTreeNode | null, no: number): [BinaryTreeNode, BinaryTreeNode] | null {
    if (node === null) {
      return null;
    }
  ...
  _infixOrder(node: BinaryTreeNode | null) {
    if (node === null) {
      return;
    }
