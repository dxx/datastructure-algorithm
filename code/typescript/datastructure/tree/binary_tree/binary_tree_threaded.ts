...
class ThreadedBinaryTreeNode {
  no: number; // 编号
  left: ThreadedBinaryTreeNode | null; // 左子节点
  right: ThreadedBinaryTreeNode | null; // 右子节点
  leftTag: number; // 左节点标记。如果 leftTag = 0, left 表示左子节点, 如果为 leftTag = 1, left 表示前驱节点
  rightTag: number; // 右节点标记。如果 rightTag = 0, right 表示右子节点, 如果为 rightTag = 1, right 表示后继节点
   ...
   */
  search(no: number) {
    let leftChildNode: ThreadedBinaryTreeNode | null = this;
...
let previous: ThreadedBinaryTreeNode | null = null; // 记录遍历时的上一个结点

/**
 * 中序线索化二叉树
 */
function infixThreadTree(node: ThreadedBinaryTreeNode | null) {
  if (!node) {
    return;
  }
  // 线索化左子节点
 ...
 * 中序遍历线索化二叉树
 */
function infixOrderThreadedTree(node: ThreadedBinaryTreeNode | null) {
