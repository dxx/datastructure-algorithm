 ...
 * 二维数组转稀疏数组
 */
function toSparseArray(array: number[][]) {
  // 统计非 0 的数量
  let count = 0;
 ...
 * 稀疏数组转二维数组
 */
function toArray(sparseArray: number[][]) {
 ...
 * 存储稀疏数组
 */
function storageSparseArray(sparseArray: number[][]) {
  let data = "";
  // 存储矩阵格式
...
function readSparseArray() {
  ...
  let sparseArray = new Array<number[]>(val + 1);
