...
class ReversePoland {
   ...
   */
  calSuffixExpression(expr: string[]) {
    let stack = new Stack<string | number>(expr.length);
   ...
   * 中缀表达式转后缀表达式
   */
  infixToSuffix(infix: string[]) {
    if (!infix) {
      return;
    }
    // 初始化两个栈，一个运算符栈 stack1 和另一个储存中间结果的栈 stack2
    let stack = new Stack<string>(infix.length);
    ...
    for (let i = 0; i < infix.length; i++) {
      ...
      if (operation) {
        ...
        while (!stack.isEmpty() && (this._priority(str, stack.peek()!) ?? 0) <= 0) {
          // 将栈顶的元素添加到 suffixes
          suffixes.push(stack.pop());
        }
        // 入栈
