...
class Chessboard {
  ...
  constructor(row: number, col: number) {
    ...
    this.visited = new Array<boolean>(length); // 标记点是否被访问过
   ...
   * startY 起始纵坐标。从 0 开始
   */
  move(startX: number, startY: number): void {
    console.log("开始走马踏棋");
   ...
   * 骑士周游算法
   */
  traversal(x: number, y: number, step: number): void {
    ...
    while (points.length != 0) {
      ...
      let p = points.shift();
      // 该点未被访问过
      if (p != undefined && !this.visited[p.y * this.col + p.x]) {
        // 继续往下走
