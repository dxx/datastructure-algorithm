...
function getBroadcast(broadcasts: Record<string, string[]>): string[] {
  let broadcastKeys: string[] = [];
  ...
  let allBroadcasts: string[] = [];
  for (let b in m) {
    // 保存所有区域
    allBroadcasts.push(b);
  }

  let selectBroadcasts: string[] = [];
  // 覆盖区域最多的电台
  let maxBroadcast = "";
  // 交集
  let tempIntersection: string[] = [];
  // 遍历所有的电台，直到所有电台被移除
...
function deleteElement(array: string[], deleteVal: string): string[] {
