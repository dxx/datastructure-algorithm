 ...
 */
function findMaxValue(w: number[], v: number[], c: number): number {
  let n = w.length;
  // 二维数组比实际物品数多出一行一列
  let value = new Array<Array<number>>(n + 1);
  // 初始化二维数组
  for (let i = 0; i < value.length; i++) {
    value[i] = new Array<number>(c + 1)
  }
...
function findMaxValue2(w: number[], v: number[], c: number): number {
  // 数组比实际物品总数多出一列
  let value = new Array<number>(c + 1);
  value.fill(0);
