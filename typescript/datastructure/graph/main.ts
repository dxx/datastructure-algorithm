...
class Graph {
  ...
  private numOfVertex: number;
  private numOfEdge: number;

  constructor(num: number) {
    this.vertexes = new Array(num);
    this.matrix = new Array(num);
    this.matrix.fill(0 as unknown as number[]);
   ...
   */
  addEdge(i1: number, i2: number, weight: number) {
   ...
   */
  dfs() {
    let isVisited = new Array<boolean>(this.getNumOfVertex());
    // 遍历所有的顶点，进行深度优先遍历
   ...
   */
  dfsRecursion(isVisited: boolean[], v: number) {
   ...
   */
  bfs2(isVisited: boolean[], v: number) {
    console.log(this.vertexes[v]);
   ...
   * 获取下一个邻接顶点下标
   */
  getNextVertex(i1: number, i2: number) {
