...
> 各种语言实现代码：[Go](./golang/algorithm/dynamicprogramming)   [Java](./java/algorithm/src/com/dxx/dynamicprogramming)   [JavaScript](./javascript/algorithm/dynamicprogramming)   [TypeScript](./typescript/algorithm/dynamicprogramming)   [Python](./python/algorithm/dynamicprogramming)   [Rust](./rust/algorithm/src/dynamic_programming)
>
> 默认使用 **Python** 语言实现。
...
```python
def find_max_value(w: list[int], v: list[int], c: int) -> int:
    n = len(w)
    # 二维数组比实际物品数多出一行一列
    # 初始化二维数组
...
```shell
❯ python -m unittest test_backpack.Test.test_find_max_value
填表如下:
0 0 0 0
0 500 500 500
0 500 5000 5500
...
```python
def find_max_value2(w: list[int], v: list[int], c: int) -> int:
    # 数组比实际物品总数多出一列
    value = [0 for _ in range(c + 1)]
...
```shell
❯ python -m unittest test_backpack.Test.test_find_max_value2
最大价值总和为: 8000
```
